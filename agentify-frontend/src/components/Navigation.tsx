...
export default function Navigation({ onOpenTerms }: NavigationProps) {
  const navigate = useNavigate();
  const [showSidebar, setShowSidebar] = useState(true);
  const isCollapsed = !showSidebar;

  // Reusable class for nav label animation
  const labelClass = `transition-all duration-200 whitespace-nowrap overflow-hidden ${
    ...
    >
      <div>
        ...
        >
          {!isCollapsed ? (
            <img
              ...
              alt="Agentify Logo"
              className="w-28 m-auto mt-2 prevent-drag"
              draggable={false}
            />
          ) : (
            <div className="absolute top-3 m-0">
              <img
                src={agentifyLogo}
                alt="Agentify Logo"
                className="relative w-7 m-auto prevent-drag"
                draggable={false}
              />
            </div>
          )}
          ...
          <button
            ...
            className={`w-8 h-8 p-1 rounded-md hover:bg-agentify-bg-gray hover:cursor-pointer
              ${isCollapsed ? 'relative top-5 m-auto mt-5' : 'mt-2 ml-0'}`}
          >
            {showSidebar ? (
              <svg
                ...
                viewBox="0 0 20 20"
              >
                <path d="M2 6a3 3 0 0 1 3-3h10a3 3 0 0 1 3 3v7a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3zm6.5-2v11H15a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zm-1 0H5a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h2.5z" />
              </svg>
            ) : (
              <svg
                className="stroke-agentify-dark-gray fill-agentify-dark-gray"
                ...
                viewBox="0 0 20 20"
              >
                <path d="M5 3a3 3 0 0 0-3 3v7a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V6a3 3 0 0 0-3-3zm10 1a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2H8.5V4z" />
              </svg>
            )}
          </button>
        </div>
        ...
        <nav className="mt-4 space-y-2">
          ...
          <button
            ...
            title="agents"
            onClick={() => navigate('/')}
            className={`w-9/10 flex items-center m-auto hover:cursor-pointer ${
              isCollapsed ? 'justify-center gap-0 px-0' : 'justify-start gap-3 px-6'
            } py-3 rounded-xl bg-agentify-light-coral text-agentify-accent-coral font-medium bg-agentify-bg-gray transition-all`}
          >
            ...
            <span className={labelClass}>agents</span>
          </button>
          ...
          <button
            ...
            className={`w-9/10 flex items-center m-auto hover:cursor-pointer ${
              isCollapsed ? 'justify-center gap-0 px-0' : 'justify-start gap-3 px-6'
            } py-3 text-agentify-dark-gray hover:bg-agentify-bg-gray rounded-xl transition-all`}
          >
            <span className="text-lg">
              <svg
                viewBox="0 0 110 110"
          ...
          >
            ...
            <span className={labelClass}>knowledgebases</span>
          </button>
        ...
        {showSidebar && (
          <div className="mt-10 px-6 transition-opacity duration-200">
      ...
      {showSidebar && (
        <div className="px-6 py-4 text-xs text-agentify-dark-gray flex transition-opacity duration-200">
          <button className="hover:underline hover:cursor-pointer m-auto" onClick={onOpenTerms}>
            Terms of service
          </button>
        </div>
      )}
